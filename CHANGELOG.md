...
- **BREAKING** `GitDir.populateBranch` - moving away from `TempDir` from
  `bot_io`
...
- **Deprecated** `GitDir.populateBranch` - moving away from `TempDir` from
  `bot_io`
- **NEW!** `GitDir.updateBranch` like `populateBranch`, but exposes a
  `Directory`
